...
Complete step-by-step guide to deploy the PlayConnect platform (API + Admin Web + User Web).
...
2. Create a new project: **PlayConnect**
...
4. **Cluster name**: `playconnect-prod`
5. Click **Create Cluster** (takes 3-5 minutes)

### 1.3 Create Database User

1. Go to **Database Access** → **Add New Database User**
2. Username: `playconnect_app`
...
3. Copy the connection string:
   ```
   mongodb+srv://playconnect_app:<password>@playconnect-prod.xxxxx.mongodb.net/playconnect?retryWrites=true&w=majority
   ```
...
3. Select the **playconnect** repository
...
2. Select your **playconnect** repository
3. Render auto-detects the `render.yaml` file
4. Review the services it will create:
   - `playconnect-api` (Web Service, Docker)
   - `playconnect-admin` (Web Service, Docker)
   - `playconnect-app` (Web Service, Docker)
   - `playconnect-redis` (Redis)
5. Click **Apply**
...
Go to **playconnect-api** → **Environment** → **Add Environment Variable**
...
| `CORS_ALLOWED_ORIGINS` | `https://playconnect-admin.onrender.com,https://playconnect-app.onrender.com` |
...
Go to **playconnect-admin** → **Environment**

| Key | Value |
|-----|-------|
| `API_BASE_URL` | `https://playconnect-api.onrender.com` (or use internal URL if available) |
| `COOKIE_SECRET` | Auto-generated by blueprint |
| `COOKIE_SALT` | Auto-generated by blueprint |

> **Render Internal Communication**: If services are in the same region, use the internal URL format: `http://playconnect-api:3001` (free, no egress charges). Check Render docs for internal hostname format.

### 4.3 Configure User Web Environment Variables

Go to **playconnect-app** → **Environment**

Same as admin web but with different cookie secrets (auto-generated separately).

| Key | Value |
|-----|-------|
| `API_BASE_URL` | `https://playconnect-api.onrender.com` |
| `COOKIE_SECRET` | Auto-generated by blueprint |
| `COOKIE_SALT` | Auto-generated by blueprint |
...
curl https://playconnect-api.onrender.com/api/v1/health/liveness
# Expected: {"status":"ok","service":"playconnect-api","time":"..."}

curl https://playconnect-api.onrender.com/api/v1/health/readiness
...
Open in browser:
- **Admin**: `https://playconnect-admin.onrender.com` → Should show login page
- **User Web**: `https://playconnect-app.onrender.com` → Should show login page

### 5.4 Test Super Admin Login

```bash
curl -X POST https://playconnect-api.onrender.com/api/v1/admin/auth/login \
...
|---------|--------------|
| `playconnect-api` | `api.yourdomain.com` |
| `playconnect-admin` | `admin.yourdomain.com` |
| `playconnect-app` | `app.yourdomain.com` |

3. Render gives you a CNAME target for each (e.g., `playconnect-api.onrender.com`)
...
|--------|------|------|-------|
| API | CNAME | `api` | `playconnect-api.onrender.com` |
| Admin | CNAME | `admin` | `playconnect-admin.onrender.com` |
| User App | CNAME | `app` | `playconnect-app.onrender.com` |
...
Go to **playconnect-api** → **Environment** → Update:
...
const ENV_CONFIG = {
  ...
  staging: {
    API_BASE_URL: 'https://playconnect-api.onrender.com',  // Or your custom domain
  },
  production: {
...
```bash
sudo mkdir -p /opt/playconnect
sudo chown $USER:$USER /opt/playconnect
```
...
```env
# ===================================================
# PlayConnect — Production Environment Configuration
# ===================================================
...
MONGODB_URI=mongodb+srv://<user>:<password>@<cluster>.mongodb.net/playconnect?retryWrites=true&w=majority
...
```env
APP_ENV=staging
MONGODB_URI=mongodb://mongo:27017/playconnect_staging?replicaSet=rs0
...
ssh-keygen -t ed25519 -C "deploy@playconnect" -f ~/.ssh/playconnect_deploy

# Copy public key to server
ssh-copy-id -i ~/.ssh/playconnect_deploy.pub ubuntu@<server-ip>

# The PRIVATE key goes into GitHub secrets
cat ~/.ssh/playconnect_deploy
```
...
```bash
cd /opt/playconnect
nano deploy/nginx/sites-enabled/playconnect.conf
```
...
```cron
0 3 * * * certbot renew --quiet --post-hook "docker exec playconnect-nginx nginx -s reload"
```
...
```bash
cd /opt/playconnect
git clone https://github.com/<your-org>/playconnect.git .
```
...
docker build -f apps/user-web/Dockerfile -t playconnect-user-web .
```
...
```
NAME                    STATUS              PORTS
playconnect-redis       Up (healthy)        6379/tcp
playconnect-api-1       Up (healthy)        3001/tcp
playconnect-api-2       Up (healthy)        3001/tcp
playconnect-api-3       Up (healthy)        3001/tcp
playconnect-admin-web   Up                  3002/tcp
playconnect-user-web    Up                  3003/tcp
playconnect-nginx       Up                  0.0.0.0:80->80/tcp, 0.0.0.0:443->443/tcp
```
...
```bash
docker exec playconnect-redis redis-cli ping
# Expected: PONG

docker exec playconnect-redis redis-cli info memory | grep used_memory_human
...
```bash
nano /opt/playconnect/scripts/health-monitor.sh
chmod +x /opt/playconnect/scripts/health-monitor.sh
```

```bash
#!/bin/bash
# PlayConnect Health Monitor
...
if [ "$STATUS" != "200" ]; then
  echo "[$(date)] ALERT: API liveness check failed (status: $STATUS)" | \
    mail -s "PlayConnect API DOWN" "$ALERT_EMAIL" 2>/dev/null
  echo "[$(date)] ALERT: API liveness failed ($STATUS)" >> /var/log/playconnect-monitor.log
fi

# Check API readiness
STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/api/v1/health/readiness" --max-time 5)
if [ "$STATUS" != "200" ]; then
  echo "[$(date)] ALERT: API readiness check failed (status: $STATUS)" >> /var/log/playconnect-monitor.log
fi

# Check Redis
PONG=$(docker exec playconnect-redis redis-cli ping 2>/dev/null)
if [ "$PONG" != "PONG" ]; then
  echo "[$(date)] ALERT: Redis not responding" >> /var/log/playconnect-monitor.log
fi

# Check disk usage
DISK_USAGE=$(df /opt/playconnect --output=pcent | tail -1 | tr -d ' %')
if [ "$DISK_USAGE" -gt 85 ]; then
  echo "[$(date)] WARN: Disk usage at ${DISK_USAGE}%" >> /var/log/playconnect-monitor.log
fi

# Check memory
FREE_MEM=$(free -m | awk '/^Mem:/ {print $7}')
if [ "$FREE_MEM" -lt 1024 ]; then
  echo "[$(date)] WARN: Free memory low (${FREE_MEM}MB)" >> /var/log/playconnect-monitor.log
fi
```
...
```cron
*/5 * * * * /opt/playconnect/scripts/health-monitor.sh
```
...
```bash
# Check memory usage
docker exec playconnect-redis redis-cli info memory

# Check connected clients
docker exec playconnect-redis redis-cli info clients

# Check BullMQ queues
docker exec playconnect-redis redis-cli llen bull:email:wait     # Pending emails
docker exec playconnect-redis redis-cli llen bull:notification:wait  # Pending notifications
```
...
# Manual backup
cd /opt/playconnect
MONGODB_URI="mongodb://mongo:27017/playconnect?replicaSet=rs0" \
  BACKUP_DIR="/backups" \
...
0 2 * * * cd /opt/playconnect && MONGODB_URI="mongodb://mongo:27017/playconnect?replicaSet=rs0" BACKUP_DIR="/backups" BACKUP_RETENTION_DAYS=7 bash scripts/backup/mongodump-backup.sh >> /var/log/playconnect-backup.log 2>&1
```
...
```bash
sudo nano /etc/logrotate.d/playconnect
```

```
/var/log/playconnect*.log {
    daily
    rotate 14
    compress
    delaycompress
...
# Watch Redis memory
watch -n5 'docker exec playconnect-redis redis-cli info memory | grep used_memory_human'

# Watch MongoDB connections
watch -n5 'docker exec playconnect-redis redis-cli dbsize'
```
...
| Redis | `docker exec playconnect-redis redis-cli ping` |
| MongoDB (staging) | `docker exec playconnect-mongo mongosh --eval "rs.status()"` |
| BullMQ email queue | `docker exec playconnect-redis redis-cli llen bull:email:wait` |
...
| Mongo shell | `docker exec -it playconnect-mongo mongosh` |

### Redis

| Action | Command |
|--------|---------|
| Memory usage | `docker exec playconnect-redis redis-cli info memory` |
| Client count | `docker exec playconnect-redis redis-cli info clients` |
| Flush cache | `docker exec playconnect-redis redis-cli FLUSHDB` |
| Queue depth | `docker exec playconnect-redis redis-cli llen bull:email:wait` |
...
# Reload nginx
docker exec playconnect-nginx nginx -s reload
```
...
docker exec playconnect-api-1 wget --quiet --spider http://localhost:3001/api/v1/health/readiness

# If readiness fails, check MongoDB:
# Atlas: Check Atlas dashboard for alerts
# Self-hosted:
docker exec playconnect-mongo mongosh --eval "rs.status()"
```
...
docker exec playconnect-redis redis-cli info stats | grep keyspace
