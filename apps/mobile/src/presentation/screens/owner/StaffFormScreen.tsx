...
import {
  ...
  updateStaff,
  listStaff,
  getStaffPhotoUploadPath,
} from '../../../infra/staff/staff-api';
...
// Backend create/update-staff DTOs restrict gender to MALE/FEMALE (intentional —
// student/enquiry use the wider GENDERS contract instead). Picker mirrors the
// narrower set so the "Other" option doesn't 400 server-side.
...
export function StaffFormScreen() {
  ...
  const [resolving, setResolving] = useState(mode === 'edit' && !paramStaff && !!staffUserId);
  ...
  if (mode === 'edit' && !staffForForm) {
    return (
      <SafeAreaView style={[styles.scroll, styles.center]}>
        ...
        <Text style={styles.resolveText}>Open the form again from the staff list.</Text>
...
function StaffFormBody({ mode, staff }: StaffFormBodyProps) {
  ...
  const [whatsappNumber, setWhatsappNumber] = useState(
    stripCountryCode(staff?.whatsappNumber ?? ''),
  );
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        if (normalizeToE164(phoneNumber.trim()) !== staff?.phoneNumber)
          patch.phoneNumber = normalizeToE164(phoneNumber.trim());
          ...
          <View style={styles.chipRow}>
            {SALARY_FREQ_OPTIONS.map((opt) => (
              ...
              >
                ...
                <Text
                  style={[styles.chipText, salaryFrequency === opt.value && styles.chipTextActive]}
                >
                  {opt.label}
                </Text>
              </Pressable>
