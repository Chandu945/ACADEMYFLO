...
export class GenerateIdCardUseCase {
  ...
  async execute(input: GenerateIdCardInput): Promise<Result<{ buffer: Buffer; filename: string }, AppError>> {
    ...
    try {
      ...
      // Academy header
      doc.fontSize(11).font('AppSans-Bold').fillColor('#2563eb')
      ...
      // Student name
      doc.fontSize(12).font('AppSans-Bold').fillColor('#000000')
        .text(student.fullName, 15, doc.y, { width: 310, align: 'center' });
      doc.moveDown(0.5);

      // Details
      doc.font('AppSans').fontSize(8).fillColor('#333333');
      const detailY = doc.y;
      const leftCol = 15;

      const addIdField = (label: string, value: string, yPos: number) => {
        doc.font('AppSans-Bold').text(`${label}:`, leftCol, yPos, { continued: true, width: 310 });
        doc.font('AppSans').text(` ${value}`);
      };
