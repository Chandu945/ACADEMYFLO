...
export class HandleCashfreeWebhookUseCase {
  ...
  async execute(
    ...
    // 1b. Replay detection. Tolerance widened to 120s — a tighter 60s window
    // was rejecting legitimate webhooks any time the server clock drifted
    // (Render instances aren't always tightly NTP-synced). 120s still keeps
    // the replay attack surface small while surviving normal cloud clock
    // jitter. We also warn on skew > 30s so clock-drift problems surface in
    // logs early instead of silently ageing into hard rejections.
    ...
    if (skew > WEBHOOK_SKEW_WARN_SECONDS) {
      this.logger.warn('Webhook clock skew is high — check NTP sync', {
        skewSeconds: skew,
      });
    }
