...
export default function EnquiriesError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error('[EnquiriesPage] Render error:', error); }, [error]);
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '64px 24px', gap: '16px' }}>
      <h2 style={{ fontSize: '20px', fontWeight: 600 }}>Failed to load enquiries</h2>
