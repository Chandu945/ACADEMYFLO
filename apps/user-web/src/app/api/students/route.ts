...
export async function GET(request: NextRequest) {
  ...
  const params = buildSafeParams({
    ...
    feeFilter: searchParams.get('feeFilter') || undefined,
...
export async function POST(request: NextRequest) {
  ...
  if (!result.ok) return NextResponse.json({ message: result.error.message, fieldErrors: result.error.fieldErrors }, { status: 400 });
