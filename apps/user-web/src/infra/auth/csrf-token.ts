...
export async function setCsrfCookie(): Promise<string> {
  ...
  cookieStore.set(COOKIE_NAME, token, {
    // httpOnly: false so the client can read it and echo it back in the header.
    httpOnly: false,
    sameSite: 'lax',
 ...
 * matches the af_user_csrf cookie. Both must be present and equal
 * (constant-time compare to avoid timing oracles).
 *
 * Call this on every state-changing route before touching state.
 */
